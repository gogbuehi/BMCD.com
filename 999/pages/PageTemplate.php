 ...
 */
class PageTemplate extends Bmcd999Page {
    ...
    const INVENTORY_IDENTIFIER='_999_inventory_';
    const MODEL_LINEUP_IDENTIFIER='_999_model_lineup_';
    ...
    static function getHtmlString($type,$html,$uniqueValue) {
        ...
        switch($type) {
            ...
            case self::PLACEHOLDER_TYPE_PAGE_TITLE:
                ...
                $tag = 'title';
                $attributeArray = '';
                $htmlPlaceholder = "<title>Lamborghini San Francisco, Lotus San Francisco, Bentley San Francisco</title>";
                break;
            case self::PLACEHOLDER_TYPE_HEADER:
                ...
                $tag = 'div';
                $attributeArray = array('class' => 'ModBMCD999Header');
                break;
            case self::PLACEHOLDER_TYPE_FOOTER:
                ...
                $tag = 'div';
                $attributeArray = array('class' => 'ModBMCD999Footer');
                break;
            default:
    ...
    static function getAllDynamicModules($html,$placeholdArray) {
        global $tLog;
        if (!is_array($placeholdArray)) {
            $msg = "PlaceholderArray is not an array. It is an(".get_class($placeholdArray).")";
            $tLog->warn($msg);
            throw new Exception($msg);
        }
        $infos = array();
        foreach ($placeholdArray as
            $key =>
            $value) {
            if ($value) {
                $info = true;

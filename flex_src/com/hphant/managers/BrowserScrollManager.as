package com.hphant.managers
{
	...
	[Event(name="change",type="com.hphant.events.BrowserScrollEvent")]
	public class BrowserScrollManager extends EventDispatcher
	{
		private static var _instance:BrowserScrollManager;
		public function BrowserScrollManager()
		{
			super(this);
			if(_instance){
		...
		public static function getInstance():BrowserScrollManager{
			if(!_instance){
				_instance = new BrowserScrollManager();
			}
			return _instance;
		}
		...
		private function setScrollPositionCallback(x:Number,y:Number,width:Number,height:Number):void{
			...
			this._w = width;
			this._h = height;
			this.dispatchEvent(new BrowserScrollEvent(BrowserScrollEvent.CHANGE,_w,_h,_vscroll,_hscroll));
		}

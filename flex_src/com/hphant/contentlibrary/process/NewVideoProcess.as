package com.hphant.contentlibrary.process
{
	...
	public class NewVideoProcess extends EventDispatcher implements IImageProcess, IResponder
	{
		public function NewVideoProcess()
		{
			super(this);
		}
		...
		private function commitMaster(event:Event):void
		{
			...
			if(!this._master.description || !this._master.title || !this._master.alternate){
				...
				var newImage:VideoEvent = new VideoEvent(VideoEvent.ADD_VIDEO);
		...
		public function loadStep(event:Event):void{
			...
			if(event.currentTarget.data is Video){
				...
				log("Video Scale = "+this._master.scale);
		...
		{
			var alert:AlertPallet = AlertPallet.show("Video sucessfully added.",
														 "Success",
														 Alert.OK,
														 Alert.OK);
			log(event);
		...
		public function handleFault (event:GeneralEvent, fault:Fault)	: void
		{
				...
				v.image = true;
				this.pallet.setView(v);
			var alert:AlertPallet = AlertPallet.show("Video add failed. Please try again.",
														 "Add Failed",
														 Alert.OK,
														 Alert.OK);
			log(event);
		...
		protected function log(message:Object,level:uint=0):void{
			Logger.log("[NewVideoProcess] "+message,level)
		}
